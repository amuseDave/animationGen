...
export function handleTranslateInputs(val, max) {
  ...
  if (Math.abs(val) > max) val = val > 0 ? max : -max;
...
export function convertTranslateValueToOffSetXY(val) {
  return val + 125 + 10 >= 250 ? 250 - 10 : val + 115 <= 0 ? 0 + 10 : val + 125;
}
